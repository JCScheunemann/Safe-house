...
wire [TAM+TAM-1:0] beta   [0:(TAM>>1)-1];  // MD ou menosMD
wire [TAM+TAM-1:0] beta0  [0:(TAM>>1)-1];  // doisMD ou menos2MD
wire [TAM+TAM-1:0] beta1  [0:(TAM>>1)-1];  // atraves de xor1 escolhe 2A ou A
wire [TAM+TAM-1:0] beta2  [0:(TAM>>1)-1];  // atraves da flag escolhe beta1 e 0
...
generate
  for (I=1; I<TAM+1 ; I=I+2) begin: booth      //radix 2, soma TAM/2 vezes; 0, 2, 4,8...
    ...
    assign beta [(I>>1)] = MD ;     //manda md ou ~md, falta somar 1
    assign beta0[(I>>1)] = doisMD ; //manda 2md ou ~2md, falta somar 1
    assign beta1[(I>>1)] = xor1[I>>1] ? beta[(I>>1)]: beta0[(I>>1)];//(MD^ MR[I+1]) + MR[I+1] : (doisMD^ MR[I+1]) + MR[I+1]; //seleciona se eh +-md ou +-2md
    //assign beta1[(I>>1)] = (beta0[I>>1] ^ MR[I+1]) + MR[I+1]; // complementa 1
    assign beta2[(I>>1)] = {(TAM+TAM){flag[I>>1]}}  &  ( ({(TAM+TAM){MR[I+1]}} ^ beta1[(I>>1)] )+ MR[I+1]);           //se todos iguais zera a parcial
    assign beta3[(I>>1)] = beta2[I>>1] << (I-1) ;   //TODO trocar deslocamento por concatenacao    //rotaciona i-1 vezes, segundo logica booth modificado,
    assign P  [(I>>1)] = beta3[I>>1];  // atualiza valor da soma parcial no vetor P [1->1],[3->2],[5->3],...[15->8]
    assign Cin[(I>>1)] = 1'b0;//  MR[I+1];
  end
...
generate //soma as parciais
	for (J=0; J<=(TAM>>1)-1; J=J+1) begin: sums //numero de operandos sempre e' metade do tamanho de bits
		//inicio somador parametrizados
    case (J)
        0:   assign Spar[J]=P[J];
        default: somador #(.TAM(TAM+TAM)) soma(
                //.Cin(Cin[J]),
    						.A(Spar[J-1]),
    						.B(P[J]),
    						.S(Spar[J] )
